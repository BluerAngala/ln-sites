...
export interface LarkListParams {
  ...
  pageToken?: string;
  viewId?: string;
  filter?: { conjunction: 'and' | 'or'; conditions: Array<{ field_name: string; operator: string; value: unknown[] }> };
...
export interface LarkListResponse<T = Record<string, unknown>> {
  items: Array<{ record_id: string; fields: T; created_time?: number; last_modified_time?: number }>;
  hasMore: boolean;
  pageToken?: string;
  total: number;
}
...
export async function listRecords<T = Record<string, unknown>>(
  ...
  const data = await res.json() as {
    ...
    data?: { items?: Array<{ record_id: string; fields: T; created_time?: number; last_modified_time?: number }>; has_more?: boolean; page_token?: string; total?: number };
  };
  if (data.code !== 0) {
