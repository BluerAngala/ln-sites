...
function saveDiskCache(bundle: ContentBundle) {
  ...
  try {
    writeFileSync(CACHE_FILE, JSON.stringify(bundle, null, 2));
  } catch {}
}
...
const i18nStr = (zh: unknown, en?: unknown): I18n<string> => ({
  zh: String(zh ?? ''),
  en: en ? String(en) : undefined,
});
...
function mapCase(rec: LarkRecord): CaseItem {
  ...
  return {
    ...
    background: i18nStr(f.background, f.backgroundEn),
    approach: i18nStr(f.approach, f.approachEn),
    result: i18nStr(f.result, f.resultEn),
    outcome: (f.outcome as CaseItem['outcome']) ?? 'win',
    extraMeta: {
      caseNo: typeof f.caseNo === 'string' ? f.caseNo : undefined,
      amount: typeof f.amount === 'string' ? f.amount : undefined,
      stats: typeof f.stats === 'string' ? f.stats : undefined,
    },
  };
}
...
function mapPractice(rec: LarkRecord): Practice {
  ...
  return {
    ...
    order: typeof f.order === 'number' ? f.order : 999,
    aliases: f.aliases ? i18nStrA(f.aliases, f.aliasesEn) : undefined,
  };
}
...
async function loadFromLark(): Promise<ContentBundle> {
  ...
  const [lawyersRaw, casesRaw, newsRaw, honorsRaw, practicesRaw, officesRaw, socialRaw, configRaw] =
    await Promise.all([
...
export async function getCases(
  opts: { practiceArea?: string; featured?: boolean; limit?: number } = {},
): Promise<CaseItem[]> {
  let list = (await getContent()).cases.filter((c) => c.status === 'published');
  if (opts.practiceArea)
    list = list.filter(
      (c) => c.practiceArea?.zh === opts.practiceArea || c.practiceArea?.en === opts.practiceArea,
    );
...
export async function getRelatedCases(slug: string, limit = 3): Promise<CaseItem[]> {
  const all = await getCases();
  const cur = all.find((c) => c.slug === slug);
  if (!cur) return [];
  const area = cur.practiceArea?.zh;
  const sameArea = all.filter((c) => c.slug !== slug && c.practiceArea?.zh === area);
  if (sameArea.length >= limit) return sameArea.slice(0, limit);
  const others = all.filter((c) => c.slug !== slug && c.practiceArea?.zh !== area);
...
export async function getNews(
  ...
  if (opts.category)
    list = list.filter((n) => n.category.zh === opts.category || n.category.en === opts.category);
